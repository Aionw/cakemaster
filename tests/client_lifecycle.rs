...
fn rejects_invalid_configuration_and_nil_activation() {
    ...
    assert_eq!(
        ClientRegistry::with_config(ClientLifecycleConfig::new(8).with_cleanup_scan_budget(0))
            .err(),
        Some(ClientLifecycleConfigError::ZeroCleanupScanBudget)
    );
...
fn repeated_activation_reuses_generation_and_refreshes_deadline() {
    ...
    assert!(
        registry
            .claim_due_cleanups(ClientTick::new(10), 8)
            .is_empty()
    );
    assert_eq!(registry.state(client(1)), Some(ClientState::Active));

    let cleanup = registry.claim_due_cleanups(ClientTick::new(15), 8);
...
fn heartbeat_deadline_is_monotonic_and_old_heap_record_is_reordered() {
    ...
    assert!(
        registry
            .claim_due_cleanups(ClientTick::new(20), 8)
            .is_empty()
    );
    assert!(
        registry
            .claim_due_cleanups(ClientTick::new(24), 8)
            .is_empty()
    );
    assert_eq!(registry.state(client(1)), Some(ClientState::Active));

    let cleanup = registry.claim_due_cleanups(ClientTick::new(25), 8);
    assert_eq!(cleanup.len(), 1);
    assert_eq!(cleanup[0].session(), session);
}
...
fn heartbeat_at_deadline_fences_session_until_cleanup_finishes() {
    ...
    let cleanup = registry.claim_due_cleanups(ClientTick::new(5), 8);
    assert_eq!(cleanup.len(), 1);
    assert_eq!(cleanup[0].session(), session);
    assert_eq!(cleanup[0].reason(), CleanupReason::HeartbeatExpired);
    assert!(
        registry
            .claim_due_cleanups(ClientTick::new(5), 8)
            .is_empty()
    );

    cleanup.into_iter().next().unwrap().finish().unwrap();
    let next = registry
...
#[test]
fn drain_is_exclusive_and_finishing_allows_a_new_generation() {
    let registry = registry(10, 8);
    let old = registry
        .activate(client(1), ClientTick::ZERO)
        .unwrap()
...
#[test]
fn dropped_cleanup_claim_is_retried_by_due_cleanup_scan() {
    let registry = registry(5, 8);
    let session = registry
        .activate(client(1), ClientTick::ZERO)
        .unwrap()
    ...
    let retry = registry.claim_due_cleanups(ClientTick::new(5), 8);
    assert_eq!(retry.len(), 1);
    assert_eq!(retry[0].session(), session);
    retry.into_iter().next().unwrap().finish().unwrap();
    assert!(registry.is_empty());
}

#[test]
fn cleanup_budget_leaves_remaining_due_clients_for_later_rounds() {
    ...
    let mut cleanups = registry.claim_due_cleanups(ClientTick::new(5), 8);
    ...
    let next = registry.claim_due_cleanups(ClientTick::new(5), 8);
    assert_eq!(next.len(), 1);
    cleanups.extend(next);
    let next = registry.claim_due_cleanups(ClientTick::new(5), 8);
    assert_eq!(next.len(), 1);
    cleanups.extend(next);
    assert!(
